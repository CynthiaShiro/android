...
class SqliteDatabase internal constructor(context: Context?) :
    ...
    fun listContacts(): ArrayList<person> {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                storeContacts.add(person(id, name, phno))
            }
    ...
    fun addContacts(contacts: person) {
        val values = ContentValues()
        values.put(COLUMN_NAME, contacts.hospitalname)
        values.put(COLUMN_NO, contacts.bloodgroup)
        val db = this.writableDatabase
        db.insert(TABLE_CONTACTS, null, values)
    }
    fun updateContacts(contacts: person) {
        val values = ContentValues()
        values.put(COLUMN_NAME, contacts.hospitalname)
        values.put(COLUMN_NO, contacts.bloodgroup)
        val db = this.writableDatabase
